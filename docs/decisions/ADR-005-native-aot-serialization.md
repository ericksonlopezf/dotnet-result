...
We designed `EricksonLopez.Result.Serialization` with NativeAOT-safe `System.Text.Json` custom converters (`ResultJsonConverterFactory`, `ErrorJsonConverter`) and pre-generated `JsonSerializerContext` definitions (`ResultJsonSerializerContext`).
...
### Negative / Trade-Offs
- NativeAOT scenarios require consumer code to register custom generic `Result<T>` types in their application `JsonSerializerContext`.

...
The constructor is marked with `[Obsolete(error: false)]` — an IDE-visible warning — accompanied by `[RequiresUnreferencedCode]` and `[RequiresDynamicCode]` with comprehensive messages.

**`[Obsolete(error: true)]` is not used.**

...
4. **`ErrorBuilder.Build()`:** Explicitly excluded in `stryker-config.json` via global `ignore-methods`.
5. **String Mutations:** Excluded globally in `stryker-config.json` via `ignore-mutations: ["string"]`.
   ...
   **Why global exclusion is preferred over 400+ inline comments:**
   Adding `// Stryker disable once string` inline comments to every string literal in the codebase
   would produce over 400 annotation lines across all source files. This noise:
   - Harms code readability by mixing Stryker directives into business logic
   - Makes it harder to identify genuine `// Stryker disable` usages that silence real coverage gaps
   - Provides no additional safety compared to the global exclusion
...
* **Negative:** Mutating logic inside `ErrorBuilder.Build()` will not fail the CI. We accept this technical debt and rely on standard unit test coverage and code review for this specific method.

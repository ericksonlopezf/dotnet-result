...
**Documentation**: This characteristic is documented in `ResultExceptionBehavior.cs` XML docs at lines 33-42 and in the package README performance section.
...
- `ResultExceptionBehavior` has a cold start per handler type (documented above in §11).

## Related

- ADR-016 — `IResultOutcome` interface decision (boxing tradeoffs and performance guidance)
- Architecture Audit Walkthrough (2026-07-31)
- ARB Audit Report (2026-08-01): `ARB_REPORT_ERICKSONLOPEZ_RESULT.md`
- Independent Architecture Audit Reports (v0.1.0-preview.1)
- preview.5 Corrections: `CHANGELOG.md` entry `[1.0.0-preview.5]` for full list of changes applied in the third ARB pass

...
public class ResultExtensionsTaskNonGenericTests
{
    private static readonly Error SharedError = Error.Failure("E", "M");

    private static Task<Result> S() => Task.FromResult(Result.Success());
    private static Task<Result> F() => Task.FromResult(Result.Failure(SharedError));
    private static async Task<Result> IS() { await Task.Yield(); return Result.Success(); }
    private static async Task<Result> IF() { await Task.Yield(); return Result.Failure(SharedError); }
    ...
    [Fact]
    public async Task Bind_FuncTaskResult()
    {
        (await S().Bind(() => Task.FromResult(Result.Success()))).ShouldBeSuccess();
        (await IS().Bind(() => Task.FromResult(Result.Success()))).ShouldBeSuccess();
        (await F().Bind(() => Task.FromResult(Result.Success()))).ShouldBeFailure();
        (await IF().Bind(() => Task.FromResult(Result.Success()))).ShouldBeFailure();
    }
    ...
    [Fact]
    public async Task Bind_TNext_FuncTaskResultTNext()
    {
        (await S().Bind(() => Task.FromResult(Result.Success(42)))).ShouldBeSuccess().Should().Be(42);
        (await IS().Bind(() => Task.FromResult(Result.Success(42)))).ShouldBeSuccess().Should().Be(42);
        (await F().Bind(() => Task.FromResult(Result.Success(42)))).ShouldBeFailure();
        (await IF().Bind(() => Task.FromResult(Result.Success(42)))).ShouldBeFailure();
    }
    ...
    [Fact]
    public async Task TapOnSuccess_FuncTask()
    {
        int count = 0;
        (await S().TapOnSuccess(() => { count++; return Task.CompletedTask; })).ShouldBeSuccess();
        Assert.Equal(1, count); count = 0;
        (await IS().TapOnSuccess(() => { count++; return Task.CompletedTask; })).ShouldBeSuccess();
        Assert.Equal(1, count); count = 0;
        (await F().TapOnSuccess(() => { count++; return Task.CompletedTask; })).ShouldBeFailure();
        Assert.Equal(0, count); count = 0;
        (await IF().TapOnSuccess(() => { count++; return Task.CompletedTask; })).ShouldBeFailure();
        Assert.Equal(0, count);
    }
    ...
    [Fact]
    public async Task TapOnFailure_FuncTask()
    {
        int count = 0;
        (await F().TapOnFailure(e => { count++; return Task.CompletedTask; })).ShouldBeFailure();
        Assert.Equal(1, count); count = 0;
        (await IF().TapOnFailure(e => { count++; return Task.CompletedTask; })).ShouldBeFailure();
        Assert.Equal(1, count); count = 0;
        (await S().TapOnFailure(e => { count++; return Task.CompletedTask; })).ShouldBeSuccess();
        Assert.Equal(0, count); count = 0;
        (await IS().TapOnFailure(e => { count++; return Task.CompletedTask; })).ShouldBeSuccess();
        Assert.Equal(0, count);
    }
    ...
    [Fact]
    public async Task Recover_FuncTaskResult()
    {
        (await F().Recover(e => Task.FromResult(Result.Success()))).ShouldBeSuccess();
        (await IF().Recover(e => Task.FromResult(Result.Success()))).ShouldBeSuccess();
        (await S().Recover(e => Task.FromResult(Result.Failure(Error.Failure("2", "2"))))).ShouldBeSuccess();
        (await IS().Recover(e => Task.FromResult(Result.Failure(Error.Failure("2", "2"))))).ShouldBeSuccess();
    }
}

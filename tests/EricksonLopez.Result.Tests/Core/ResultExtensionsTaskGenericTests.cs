...
public class ResultExtensionsTaskGenericTests
{
    ...
    [Fact]
    public async Task Bind_FuncTaskResultTNext()
    {
    ...
    {
        int count = 0;
        (await S().TapOnSuccess(v => { count += v; return Task.CompletedTask; })).ShouldBeSuccess();
        Assert.Equal(42, count); count = 0;
        (await IS().TapOnSuccess(v => { count += v; return Task.CompletedTask; })).ShouldBeSuccess();
        Assert.Equal(42, count); count = 0;
        (await F().TapOnSuccess(v => { count += v; return Task.CompletedTask; })).ShouldBeFailure();
        Assert.Equal(0, count); count = 0;
        (await IF().TapOnSuccess(v => { count += v; return Task.CompletedTask; })).ShouldBeFailure();
        Assert.Equal(0, count);
    }

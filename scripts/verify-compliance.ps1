...
.DESCRIPTION
    ...
    9. Markdown relative link integrity and Linux case-sensitivity across repository.
#>
...
$missingHeaders = 0
$mangledHeaders = 0
$headerFiles = Get-ChildItem -Path (Join-Path $targetRoot "src"), (Join-Path $targetRoot "tests"), (Join-Path $targetRoot "scripts"), (Join-Path $targetRoot ".github/workflows") -Recurse -Include *.cs, *.ps1, *.js, *.yml | Where-Object { $_.FullName -notmatch '[\\/](obj|bin|\.git|StrykerOutput|BenchmarkDotNet\.Artifacts)[\\/]' }
foreach ($hf in $headerFiles) {
    $firstLine = (Get-Content $hf.FullName -TotalCount 1 -Encoding UTF8 -ErrorAction SilentlyContinue)
    if ($firstLine -notmatch "Copyright .* Erickson Lopez.* MIT License") {
        Write-Host "  ❌ Missing canonical MIT header in $($hf.FullName)" -ForegroundColor Red
        $violations++
        $missingHeaders++
    }
    if ($firstLine -match "Â©") {
        Write-Host "  ❌ Mangled copyright character 'Â©' in $($hf.FullName)" -ForegroundColor Red
        $violations++
        $mangledHeaders++
    }
}
if ($missingHeaders -eq 0 -and $mangledHeaders -eq 0) { Write-Host "  ✅ All source, script, and workflow files contain the verified canonical MIT copyright header." -ForegroundColor Green }

# 4. One Type Per File in src/
Write-Host "`n[4/8] Checking 'One Type Per File' rule in src/..." -ForegroundColor Yellow
$multiTypeFiles = 0
foreach ($cs in $srcCsFiles) {
    $content = Get-Content $cs.FullName
    $typeDecls = $content | Select-String -Pattern "^\s*(public|internal|protected)\s+(sealed\s+|readonly\s+|abstract\s+|static\s+)*(class|struct|record|interface|enum|delegate)\s+([A-Za-z0-9_]+)"
    if ($typeDecls.Count -gt 1) {
        Write-Host "  ❌ Multiple top-level types in $($cs.FullName):" -ForegroundColor Red
        foreach ($td in $typeDecls) {
            Write-Host "     Line $($td.LineNumber): $($td.Line.Trim())" -ForegroundColor DarkRed
        }
        $violations++
        $multiTypeFiles++
    }
...
foreach ($f in $allTrackedFiles) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        if ($lines[$i] -match "github\.com/ericksonlopez/dotnet-result\b") {
...
if ($badLinks -eq 0) { Write-Host "  ✅ All GitHub URLs correctly target ericksonlopezf/dotnet-result." -ForegroundColor Green }
...
if (-not $strykerConfigFiles -or $strykerConfigFiles.Count -eq 0) {
    ...
    $runStrykerScript = Join-Path $targetRoot "scripts/run-stryker.ps1"
    if (Test-Path $runStrykerScript) {
        $runContent = Get-Content $runStrykerScript -Raw
        foreach ($sf in $strykerProfiles) {
            if ($runContent -notmatch [regex]::Escape($sf.Name)) {
                Write-Host "  ❌ Stryker configuration '$($sf.Name)' is missing from scripts/run-stryker.ps1 package map." -ForegroundColor Red
                if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
                if (Get-Variable -Name "Violations" -Scope 0 -ErrorAction SilentlyContinue) { if ($Violations -is [System.Collections.IList]) { $Violations.Add("Stryker configuration '$($sf.Name)' is missing from scripts/run-stryker.ps1") } else { $Violations++ } }
                if (Get-Variable -Name "FailedChecks" -Scope 0 -ErrorAction SilentlyContinue) { $FailedChecks++ }
                $strykerErrors++
            }
        }
...
if ($strykerErrors -eq 0) {
    Write-Host "  ✅ Stryker.NET configuration, 100/98/95 thresholds, concurrency 2, anti-gaming blacklist, package matrix, and run-stryker.ps1 synchronization verified." -ForegroundColor Green
}
...
# -----------------------------------------------------------------------------
# Documentation Link Integrity & Linux Case-Sensitivity Gate
# -----------------------------------------------------------------------------
Write-Host "`n[Gate: Link Integrity & Casing] Validating documentation links and Linux case-sensitivity..." -ForegroundColor Yellow
$linkGateErrors = 0

$mdFiles = Get-ChildItem -Path $targetRoot -Recurse -Filter "*.md" -File -ErrorAction SilentlyContinue | Where-Object {
    $_.FullName -notmatch '[\\/](bin|obj|\.git|node_modules|StrykerOutput|BenchmarkDotNet\.Artifacts)[\\/]'
}
...
foreach ($file in $mdFiles) {
    ...
    foreach ($m in $matches) {
        $link = $m.Groups[1].Value.Trim().Trim('"', "'")

        # Ignore web protocols, mailto, in-page anchors, GitHub expressions, or file URIs
        if ($link -match '^(https?://|mailto:|#|file://|\$\{\{)') {
            continue
        }

        # Resolve path
        $resolved = [System.IO.Path]::GetFullPath([System.IO.Path]::Combine($dir, $link.Replace('/', [System.IO.Path]::DirectorySeparatorChar)))

        if (-not (Test-Path -LiteralPath $resolved)) {
            Write-Host "  ❌ Broken relative link in $($file.FullName): '$link' -> '$resolved' not found." -ForegroundColor Red
            if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
            if (Get-Variable -Name "Violations" -Scope 0 -ErrorAction SilentlyContinue) { if ($Violations -is [System.Collections.IList]) { $Violations.Add("Broken relative link in $($file.FullName): '$link'") } else { $Violations++ } }
            if (Get-Variable -Name "FailedChecks" -Scope 0 -ErrorAction SilentlyContinue) { $FailedChecks++ }
            $linkGateErrors++
            continue
        }

        # Case-sensitivity check for Linux compatibility
        $current = $dir
        $parts = $link -split '[\\/]'
        foreach ($part in $parts) {
            ...
            $children = Get-ChildItem -Path $current -Force | Where-Object { $_.Name -eq $part }
            if ($children) {
                if ($children.Name -cne $part) {
                    Write-Host "  ❌ Linux case-sensitivity mismatch in $($file.FullName): segment '$part' != '$($children.Name)' (link: '$link')" -ForegroundColor Red
                    if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
                    if (Get-Variable -Name "Violations" -Scope 0 -ErrorAction SilentlyContinue) { if ($Violations -is [System.Collections.IList]) { $Violations.Add("Case mismatch in $($file.FullName): '$part'") } else { $Violations++ } }
                    if (Get-Variable -Name "FailedChecks" -Scope 0 -ErrorAction SilentlyContinue) { $FailedChecks++ }
                    $linkGateErrors++
                    break
                }
                $current = $children.FullName
            }
        }
    }
}

if ($linkGateErrors -eq 0) {
    Write-Host "  ✅ Markdown relative links and Linux case-sensitivity verified." -ForegroundColor Green
}

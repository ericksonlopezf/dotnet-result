...
public static partial class ResultExtensions
{
    ...
    /// <returns>A value task representing the asynchronous operation. The task result contains a <see cref="Result{TNext}"/> with the mapped value on success, or the original failure.</returns>
        ...
        this ValueTask<Result<T>> resultTask, Func<T, TNext> mapper, CancellationToken cancellationToken = default)
    {
        if (resultTask.IsCompletedSuccessfully)
        {
            ...
            return new ValueTask<Result<TNext>>(resultTask.Result.Map(mapper));
        }
        return MapCore(resultTask, mapper, cancellationToken);
        static async ValueTask<Result<TNext>> MapCore(ValueTask<Result<T>> t, Func<T, TNext> m, CancellationToken ct)
        {
        ...
        this ValueTask<Result<T>> resultTask, TState state, Func<TState, T, TNext> mapper, CancellationToken cancellationToken = default)
    {
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            return new ValueTask<Result<TNext>>(resultTask.Result.Map(state, mapper));
        }
        return MapCore(resultTask, state, mapper, cancellationToken);
        static async ValueTask<Result<TNext>> MapCore(ValueTask<Result<T>> t, TState s, Func<TState, T, TNext> m, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
    ...
    {
        // Stryker disable once Block : Fast path optimization
        if (resultTask.IsCompletedSuccessfully)
        {
        ...
        return MapCore(resultTask, mapper, cancellationToken);
        static async ValueTask<Result<TNext>> AwaitMapValue(ValueTask<TNext> v) => Result.Success(await v.ConfigureAwait(false));
        static async ValueTask<Result<TNext>> MapCore(ValueTask<Result<T>> t, Func<T, ValueTask<TNext>> m, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
    ...
    /// <summary>
    /// Maps the success value of a <see cref="ValueTask{TResult}"/> wrapping a <see cref="Result{T}"/> using an async mapper with captured <typeparamref name="TState"/> to avoid closure allocations.
    /// </summary>
    /// <typeparam name="TState">The type of the state data forwarded to the delegate.</typeparam>
    /// <typeparam name="T">The type of the source result value.</typeparam>
    /// <typeparam name="TNext">The type of the transformed value.</typeparam>
        ...
        this ValueTask<Result<T>> resultTask, TState state, Func<TState, T, ValueTask<TNext>> mapper, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result<TNext>> AwaitMapStateValue(TState s, T value, Func<TState, T, ValueTask<TNext>> m)
            => Result.Success(await m(s, value).ConfigureAwait(false));
        static async ValueTask<Result<TNext>> MapStateCore(ValueTask<Result<T>> t, TState s, Func<TState, T, ValueTask<TNext>> m, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            var r = await t.ConfigureAwait(false);
            if (r.IsFailure) return Result.Failure<TNext>(r.Error);
    ...
    /// <param name="cancellationToken">A token that can be used to cancel the asynchronous operation.</param>
    /// <returns>A value task representing the asynchronous operation. The task result contains the bound <see cref="Result{TNext}"/> on success, or the original failure.</returns>
        ...
        this ValueTask<Result<T>> resultTask, Func<T, Result<TNext>> bind, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result<TNext>> BindCore(ValueTask<Result<T>> t, Func<T, Result<TNext>> b, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
        ...
        this ValueTask<Result<T>> resultTask, TState state, Func<TState, T, Result<TNext>> bind, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result<TNext>> BindCore(ValueTask<Result<T>> t, TState s, Func<TState, T, Result<TNext>> b, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
        ...
        this ValueTask<Result<T>> resultTask, Func<T, ValueTask<Result<TNext>>> bind, CancellationToken cancellationToken = default)
    {
        ...
        return BindCore(resultTask, bind, cancellationToken);
        static async ValueTask<Result<TNext>> BindCore(ValueTask<Result<T>> t, Func<T, ValueTask<Result<TNext>>> b, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            var r = await t.ConfigureAwait(false);
            if (r.IsFailure) return Result.Failure<TNext>(r.Error);
    ...
    /// <param name="bind">The binding function applied to produce the next result.</param>
    /// <param name="cancellationToken">A token that can be used to cancel the asynchronous operation.</param>
    /// <returns>A value task representing the asynchronous operation. The task result contains the result of the binding operation.</returns>
        ...
        this ValueTask<Result<T>> resultTask, Func<T, ValueTask<Result>> bind, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result> BindAsyncCore(ValueTask<Result<T>> t, Func<T, ValueTask<Result>> b, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            var r = await t.ConfigureAwait(false);
        ...
        this ValueTask<Result<T>> resultTask, Func<T, Result> bind, CancellationToken cancellationToken = default)
    {
        ...
        return BindCore(resultTask, bind, cancellationToken);
        static async ValueTask<Result> BindCore(ValueTask<Result<T>> t, Func<T, Result> b, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            return (await t.ConfigureAwait(false)).Bind(b);
        }
        ...
        this ValueTask<Result<T>> resultTask, TState state, Func<TState, T, Result> bind, CancellationToken cancellationToken = default)
    {
        ...
        return BindCore(resultTask, state, bind, cancellationToken);
        static async ValueTask<Result> BindCore(ValueTask<Result<T>> t, TState s, Func<TState, T, Result> b, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            return (await t.ConfigureAwait(false)).Bind(s, b);
        }
    ...
    /// <returns>A value task representing the asynchronous operation. The task result contains the value produced by the invoked branch.</returns>
        ...
        Func<T, TOut> onSuccess, Func<Error, TOut> onFailure)
    {
        ...
        static async ValueTask<TOut> MatchCore(ValueTask<Result<T>> t, Func<T, TOut> s, Func<Error, TOut> f)
        ...
        Func<TState, T, TOut> onSuccess, Func<TState, Error, TOut> onFailure)
    {
        ...
        static async ValueTask<TOut> MatchCore(ValueTask<Result<T>> t, TState s, Func<TState, T, TOut> onSucc, Func<TState, Error, TOut> onFail)
        ...
        Action<T> onSuccess, Action<Error> onFailure, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask ExecuteCore(ValueTask<Result<T>> t, Action<T> s, Action<Error> f, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
        ...
        Action<TState, T> onSuccess, Action<TState, Error> onFailure, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask ExecuteCore(ValueTask<Result<T>> t, TState st, Action<TState, T> s, Action<TState, Error> f, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
    ...
    /// <param name="cancellationToken">A token that can be used to cancel the asynchronous operation.</param>
    /// <returns>A value task representing the asynchronous operation. The task result contains the original result.</returns>
        ...
        this ValueTask<Result<T>> resultTask, Action<T> action, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result<T>> TapCore(ValueTask<Result<T>> t, Action<T> a, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
        ...
        this ValueTask<Result<T>> resultTask, TState state, Action<TState, T> action, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result<T>> TapCore(ValueTask<Result<T>> t, TState s, Action<TState, T> a, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
        ...
        this ValueTask<Result<T>> resultTask, Func<T, ValueTask> action, CancellationToken cancellationToken = default)
    {
        ...
        return TapCore(resultTask, action, cancellationToken);
        static async ValueTask<Result<T>> AwaitTapValue(ValueTask v, Result<T> r)
        {
            await v.ConfigureAwait(false);
            return r;
        }
        static async ValueTask<Result<T>> TapCore(ValueTask<Result<T>> t, Func<T, ValueTask> a, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            var r = await t.ConfigureAwait(false);
        ...
        this ValueTask<Result<T>> resultTask, Action<Error> action, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result<T>> TapOnFailureCore(ValueTask<Result<T>> t, Action<Error> a, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
        ...
        this ValueTask<Result<T>> resultTask, TState state, Action<TState, Error> action, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result<T>> TapOnFailureCore(ValueTask<Result<T>> t, TState s, Action<TState, Error> a, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
        ...
        this ValueTask<Result<T>> resultTask, Func<Error, ValueTask> action, CancellationToken cancellationToken = default)
    {
        ...
        return TapOnFailureCore(resultTask, action, cancellationToken);
        static async ValueTask<Result<T>> AwaitTapOnFailureValue(ValueTask v, Result<T> r)
        {
            await v.ConfigureAwait(false);
            return r;
        }
        static async ValueTask<Result<T>> TapOnFailureCore(ValueTask<Result<T>> t, Func<Error, ValueTask> a, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            var r = await t.ConfigureAwait(false);
    ...
    /// <param name="cancellationToken">A token that can be used to cancel the asynchronous operation.</param>
    /// <returns>A value task representing the asynchronous operation. The task result contains the original result if the predicate is satisfied, or a failure.</returns>
        ...
        this ValueTask<Result<T>> resultTask, Func<T, bool> predicate, Error error, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result<T>> EnsureCore(ValueTask<Result<T>> t, Func<T, bool> p, Error e, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
        ...
        this ValueTask<Result<T>> resultTask, TState state, Func<TState, T, bool> predicate, Error error, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result<T>> EnsureCore(ValueTask<Result<T>> t, TState s, Func<TState, T, bool> p, Error e, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
        ...
        this ValueTask<Result<T>> resultTask, Func<T, ValueTask<bool>> predicate, Error error, CancellationToken cancellationToken = default)
    {
        ...
        return EnsureCore(resultTask, predicate, error, cancellationToken);
        static async ValueTask<Result<T>> AwaitEnsureValue(ValueTask<bool> v, Result<T> r, Error e)
            => await v.ConfigureAwait(false) ? r : Result.Failure<T>(e);

        static async ValueTask<Result<T>> EnsureCore(ValueTask<Result<T>> t, Func<T, ValueTask<bool>> p, Error e, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            var r = await t.ConfigureAwait(false);
    ...
    /// <summary>
    /// Applies an async predicate with captured <typeparamref name="TState"/> to the success value of a <see cref="ValueTask{TResult}"/> wrapping a <see cref="Result{T}"/>, avoiding closure allocations.
    /// </summary>
    /// <typeparam name="TState">The type of the state data forwarded to the delegate.</typeparam>
    /// <typeparam name="T">The type of the source result value.</typeparam>
    /// <param name="resultTask">The value task containing the source result.</param>
        ...
        this ValueTask<Result<T>> resultTask, TState state, Func<TState, T, ValueTask<bool>> predicate, Error error, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result<T>> AwaitEnsureStateValue(TState s, Result<T> r, Func<TState, T, ValueTask<bool>> p, Error e)
            => await p(s, r.Value).ConfigureAwait(false) ? r : Result.Failure<T>(e);

        static async ValueTask<Result<T>> EnsureStateCore(ValueTask<Result<T>> t, TState s, Func<TState, T, ValueTask<bool>> p, Error e, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            var r = await t.ConfigureAwait(false);
            if (r.IsFailure) return r;
    ...
    /// <param name="cancellationToken">A token that can be used to cancel the asynchronous operation.</param>
    /// <returns>A value task representing the asynchronous operation. The task result contains the original result on success, or the recovered result on failure.</returns>
        ...
        this ValueTask<Result<T>> resultTask, Func<Error, Result<T>> recovery, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result<T>> RecoverCore(ValueTask<Result<T>> t, Func<Error, Result<T>> r, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
        ...
        this ValueTask<Result<T>> resultTask, TState state, Func<TState, Error, Result<T>> recovery, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result<T>> RecoverCore(ValueTask<Result<T>> t, TState s, Func<TState, Error, Result<T>> r, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
        ...
        this ValueTask<Result<T>> resultTask, Func<Error, ValueTask<Result<T>>> recovery, CancellationToken cancellationToken = default)
    {
        ...
        return RecoverCore(resultTask, recovery, cancellationToken);
        static async ValueTask<Result<T>> RecoverCore(ValueTask<Result<T>> t, Func<Error, ValueTask<Result<T>>> r, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
        ...
        this ValueTask<Result<T>> resultTask, TState state, Func<TState, Error, ValueTask<Result<T>>> recovery, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result<T>> RecoverStateCore(ValueTask<Result<T>> t, TState s, Func<TState, Error, ValueTask<Result<T>>> r, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            var res = await t.ConfigureAwait(false);
            if (res.IsSuccess) return res;
    ...
    /// <param name="cancellationToken">A token that can be used to cancel the asynchronous operation.</param>
    /// <returns>A value task representing the asynchronous operation. The task result contains the original result on success, or a failure with the transformed error.</returns>
        ...
        this ValueTask<Result<T>> resultTask, Func<Error, Error> mapper, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result<T>> MapErrorCore(ValueTask<Result<T>> t, Func<Error, Error> m, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
        ...
        this ValueTask<Result<T>> resultTask, TState state, Func<TState, Error, Error> mapper, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result<T>> MapErrorCore(ValueTask<Result<T>> t, TState s, Func<TState, Error, Error> m, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
        ...
        this ValueTask<Result<T>> resultTask, Action<Result<T>> action, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result<T>> InspectCore(ValueTask<Result<T>> t, Action<Result<T>> a, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
        ...
        this ValueTask<Result<T>> resultTask, TState state, Action<TState, Result<T>> action, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result<T>> InspectCore(ValueTask<Result<T>> t, TState s, Action<TState, Result<T>> a, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
        ...
        this ValueTask<Result> resultTask, Func<Result> bind, CancellationToken cancellationToken = default)
    {
        if (resultTask.IsCompletedSuccessfully) return new ValueTask<Result>(resultTask.Result.Bind(bind));
        return BindCore(resultTask, bind, cancellationToken);
        static async ValueTask<Result> BindCore(ValueTask<Result> t, Func<Result> b, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            return (await t.ConfigureAwait(false)).Bind(b);
        }
        ...
        this ValueTask<Result> resultTask, TState state, Func<TState, Result> bind, CancellationToken cancellationToken = default)
    {
        if (resultTask.IsCompletedSuccessfully) return new ValueTask<Result>(resultTask.Result.Bind(state, bind));
        return BindCore(resultTask, state, bind, cancellationToken);
        static async ValueTask<Result> BindCore(ValueTask<Result> t, TState s, Func<TState, Result> b, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            return (await t.ConfigureAwait(false)).Bind(s, b);
        }
        ...
        this ValueTask<Result> resultTask, Func<ValueTask<Result>> bind, CancellationToken cancellationToken = default)
    {
        ...
        return BindCore(resultTask, bind, cancellationToken);
        static async ValueTask<Result> BindCore(ValueTask<Result> t, Func<ValueTask<Result>> b, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            var r = await t.ConfigureAwait(false);
            if (r.IsFailure) return r;
        ...
        this ValueTask<Result> resultTask, Func<Result<TNext>> bind, CancellationToken cancellationToken = default)
    {
        if (resultTask.IsCompletedSuccessfully) return new ValueTask<Result<TNext>>(resultTask.Result.Bind(bind));
        return BindCore(resultTask, bind, cancellationToken);
        static async ValueTask<Result<TNext>> BindCore(ValueTask<Result> t, Func<Result<TNext>> b, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            return (await t.ConfigureAwait(false)).Bind(b);
        }
        ...
        this ValueTask<Result> resultTask, TState state, Func<TState, Result<TNext>> bind, CancellationToken cancellationToken = default)
    {
        if (resultTask.IsCompletedSuccessfully) return new ValueTask<Result<TNext>>(resultTask.Result.Bind(state, bind));
        return BindCore(resultTask, state, bind, cancellationToken);
        static async ValueTask<Result<TNext>> BindCore(ValueTask<Result> t, TState s, Func<TState, Result<TNext>> b, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            return (await t.ConfigureAwait(false)).Bind(s, b);
        }
        ...
        this ValueTask<Result> resultTask, Func<ValueTask<Result<TNext>>> bind, CancellationToken cancellationToken = default)
    {
        ...
        return BindCore(resultTask, bind, cancellationToken);
        static async ValueTask<Result<TNext>> BindCore(ValueTask<Result> t, Func<ValueTask<Result<TNext>>> b, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            var r = await t.ConfigureAwait(false);
            if (r.IsFailure) return Result.Failure<TNext>(r.Error);
        ...
        Func<TOut> onSuccess, Func<Error, TOut> onFailure)
    {
        if (resultTask.IsCompletedSuccessfully) return new ValueTask<TOut>(resultTask.Result.Match(onSuccess, onFailure));
        return MatchCore(resultTask, onSuccess, onFailure);
        static async ValueTask<TOut> MatchCore(ValueTask<Result> t, Func<TOut> s, Func<Error, TOut> f)
            => (await t.ConfigureAwait(false)).Match(s, f);
    }
        ...
        Func<TState, TOut> onSuccess, Func<TState, Error, TOut> onFailure)
    {
        if (resultTask.IsCompletedSuccessfully) return new ValueTask<TOut>(resultTask.Result.Match(state, onSuccess, onFailure));
        return MatchCore(resultTask, state, onSuccess, onFailure);
        static async ValueTask<TOut> MatchCore(ValueTask<Result> t, TState st, Func<TState, TOut> s, Func<TState, Error, TOut> f)
        ...
        Action onSuccess, Action<Error> onFailure, CancellationToken cancellationToken = default)
    {
        ...
        return ExecuteCore(resultTask, onSuccess, onFailure, cancellationToken);
        static async ValueTask ExecuteCore(ValueTask<Result> t, Action s, Action<Error> f, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            (await t.ConfigureAwait(false)).Execute(s, f);
        }
        ...
        Action<TState> onSuccess, Action<TState, Error> onFailure, CancellationToken cancellationToken = default)
    {
        ...
        return ExecuteCore(resultTask, state, onSuccess, onFailure, cancellationToken);
        static async ValueTask ExecuteCore(ValueTask<Result> t, TState st, Action<TState> s, Action<TState, Error> f, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            (await t.ConfigureAwait(false)).Execute(st, s, f);
        }
        ...
        this ValueTask<Result> resultTask, Action onSuccess, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result> TapCore(ValueTask<Result> t, Action a, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            return (await t.ConfigureAwait(false)).TapOnSuccess(a);
        }
        ...
        this ValueTask<Result> resultTask, TState state, Action<TState> onSuccess, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result> TapCore(ValueTask<Result> t, TState s, Action<TState> a, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            return (await t.ConfigureAwait(false)).TapOnSuccess(s, a);
        }
        ...
        this ValueTask<Result> resultTask, Func<ValueTask> onSuccess, CancellationToken cancellationToken = default)
    {
        ...
        return TapCore(resultTask, onSuccess, cancellationToken);
        static async ValueTask<Result> AwaitTapValue(ValueTask v, Result r)
        {
            await v.ConfigureAwait(false);
            return r;
        }
        static async ValueTask<Result> TapCore(ValueTask<Result> t, Func<ValueTask> a, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            var r = await t.ConfigureAwait(false);
        ...
        this ValueTask<Result> resultTask, Action<Error> onFailure, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result> TapOnFailureCore(ValueTask<Result> t, Action<Error> a, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            return (await t.ConfigureAwait(false)).TapOnFailure(a);
        }
        ...
        this ValueTask<Result> resultTask, TState state, Action<TState, Error> onFailure, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result> TapOnFailureCore(ValueTask<Result> t, TState s, Action<TState, Error> a, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            return (await t.ConfigureAwait(false)).TapOnFailure(s, a);
        }
        ...
        this ValueTask<Result> resultTask, Func<Error, ValueTask> onFailure, CancellationToken cancellationToken = default)
    {
        ...
        return TapOnFailureCore(resultTask, onFailure, cancellationToken);
        static async ValueTask<Result> AwaitTapOnFailureValue(ValueTask v, Result r)
        {
            await v.ConfigureAwait(false);
            return r;
        }
        static async ValueTask<Result> TapOnFailureCore(ValueTask<Result> t, Func<Error, ValueTask> a, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            var r = await t.ConfigureAwait(false);
            if (r.IsFailure) await a(r.Error).ConfigureAwait(false);
        ...
        this ValueTask<Result> resultTask, Func<bool> predicate, Error error, CancellationToken cancellationToken = default)
    {
        ...
        return EnsureCore(resultTask, predicate, error, cancellationToken);
        static async ValueTask<Result> EnsureCore(ValueTask<Result> t, Func<bool> p, Error e, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            return (await t.ConfigureAwait(false)).Ensure(p, e);
        }
        ...
        this ValueTask<Result> resultTask, TState state, Func<TState, bool> predicate, Error error, CancellationToken cancellationToken = default)
    {
        ...
        return EnsureCore(resultTask, state, predicate, error, cancellationToken);
        static async ValueTask<Result> EnsureCore(ValueTask<Result> t, TState s, Func<TState, bool> p, Error e, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            return (await t.ConfigureAwait(false)).Ensure(s, p, e);
        }
        ...
        this ValueTask<Result> resultTask, Func<Error, Error> mapper, CancellationToken cancellationToken = default)
    {
        ...
        return MapErrorCore(resultTask, mapper, cancellationToken);
        static async ValueTask<Result> MapErrorCore(ValueTask<Result> t, Func<Error, Error> m, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            return (await t.ConfigureAwait(false)).MapError(m);
        }
        ...
        this ValueTask<Result> resultTask, TState state, Func<TState, Error, Error> mapper, CancellationToken cancellationToken = default)
    {
        ...
        return MapErrorCore(resultTask, state, mapper, cancellationToken);
        static async ValueTask<Result> MapErrorCore(ValueTask<Result> t, TState s, Func<TState, Error, Error> m, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            return (await t.ConfigureAwait(false)).MapError(s, m);
        }
        ...
        this ValueTask<Result> resultTask, Action<Result> action, CancellationToken cancellationToken = default)
    {
        ...
        return InspectCore(resultTask, action, cancellationToken);
        static async ValueTask<Result> InspectCore(ValueTask<Result> t, Action<Result> a, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            return (await t.ConfigureAwait(false)).Inspect(a);
        }
        ...
        this ValueTask<Result> resultTask, TState state, Action<TState, Result> action, CancellationToken cancellationToken = default)
    {
        ...
        return InspectCore(resultTask, state, action, cancellationToken);
        static async ValueTask<Result> InspectCore(ValueTask<Result> t, TState s, Action<TState, Result> a, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            return (await t.ConfigureAwait(false)).Inspect(s, a);
        }
        ...
        this ValueTask<Result> resultTask, Func<TNext> mapper, CancellationToken cancellationToken = default)
    {
        ...
        return MapCore(resultTask, mapper, cancellationToken);
        static async ValueTask<Result<TNext>> MapCore(ValueTask<Result> t, Func<TNext> m, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            return (await t.ConfigureAwait(false)).Map(m);
        }
        ...
        this ValueTask<Result> resultTask, TState state, Func<TState, TNext> mapper, CancellationToken cancellationToken = default)
    {
        ...
        return MapStateCore(resultTask, state, mapper, cancellationToken);
        static async ValueTask<Result<TNext>> MapStateCore(ValueTask<Result> t, TState s, Func<TState, TNext> m, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            return (await t.ConfigureAwait(false)).Map(s, m);
        }
    ...
    /// <param name="recovery">The recovery function producing a fallback from the error.</param>
    /// <param name="cancellationToken">A token that can be used to cancel the asynchronous operation.</param>
    /// <returns>A value task representing the asynchronous operation. The task result contains a successful result, or the recovered result on failure.</returns>
        ...
        this ValueTask<Result> resultTask, Func<Error, Result> recovery, CancellationToken cancellationToken = default)
    {
        ...
        return RecoverCore(resultTask, recovery, cancellationToken);
        static async ValueTask<Result> RecoverCore(ValueTask<Result> t, Func<Error, Result> r, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            return (await t.ConfigureAwait(false)).Recover(r);
        }
    ...
    /// <summary>
    /// If the <see cref="Result"/> is a failure, invokes <paramref name="recovery"/> with state to attempt a corrective result.
    /// </summary>
    /// <typeparam name="TState">The type of the state data forwarded to the delegate.</typeparam>
    /// <param name="resultTask">The value task containing the source result.</param>
    /// <param name="state">The state data forwarded to the delegate to avoid closure allocations.</param>
        ...
        this ValueTask<Result> resultTask, TState state, Func<TState, Error, Result> recovery, CancellationToken cancellationToken = default)
    {
        ...
        return RecoverStateCore(resultTask, state, recovery, cancellationToken);
        static async ValueTask<Result> RecoverStateCore(ValueTask<Result> t, TState s, Func<TState, Error, Result> r, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            return (await t.ConfigureAwait(false)).Recover(s, r);
        }
        ...
        this ValueTask<Result> resultTask, Func<Error, ValueTask<Result>> recovery, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<Result> RecoverAsyncCore(ValueTask<Result> t, Func<Error, ValueTask<Result>> r, CancellationToken ct)
        {
            ct.ThrowIfCancellationRequested();
            var res = await t.ConfigureAwait(false);
            if (res.IsSuccess) return res;
